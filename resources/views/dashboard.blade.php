<x-app-layout>
    ...
    <form method="post" action='/add-semester' id="add" class="hidden w-full max-w-sm">
        <div class="mr-4 fixed bottom-5 right-20 flex items-center border-b border-teal-500 py-2">
            @csrf
            <input name='name' id="name" class="rounded-md bg-transparent  border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight" type="text" placeholder="Semester" aria-label="Full name">
            <button class="flex-shrink-0 bg-teal-500 hover:bg-teal-700 border-teal-500 hover:border-teal-700 ease in text-sm border-4 text-white py-1 px-10 rounded-md" type="submit">
            Add
            </button>
        </div>
    </form>
